...
export const ChangeInput=(val)=>{
  // val.type=="range"? val.nextSibling.value=val.value : val.previousSibling.value=val.value
  val.type=="range"? val.parentElement.nextSibling.value=val.value : val.previousSibling.querySelector('input[type="range"]').value=val.value
  // console.log("Parent",val.parentElement.parentElement.querySelector('input[type="number"]'))
}
...
export const RGBtoHexa=(obj)=>{
  ...
  // console.log("Hexa",Math.round(R_Ref.current.value), Math.round(G_Ref.current.value), Math.round(B_Ref.current.value))
...
export const colorSpaceBG=(obj,stts)=>{
  ...
  // obj.CS_HSL.current.style.background='linear-gradient(90deg, '+hsl_1+','+hsl_2+')'

  const hsv_2='hsl('+obj.H.current.value+',100%,50%)'
  // obj.CS_HSV.current.style.background='linear-gradient(90deg, white,'+hsv_2+')'
...
export const HSLtoPointer=(obj,stts)=>{
  ...
  // pointer_HSL_Ref.current.style.top=Math.abs(L_Ref.current.value*2-200)-12+"px"
  // pointer_HSL_Ref.current.style.left=LS_Ref.current.value*3.6-12+"px"
  // pointer_HSV_Ref.current.style.top=Math.abs(V_Ref.current.value*2-200)-12+"px"
  // pointer_HSV_Ref.current.style.left=VS_Ref.current.value*3.6-12+"px"

  stts.setPointer([
      Math.abs(obj.L.current.value*2-200)-12+"px",
      obj.LS.current.value*3.6-12+"px",
      Math.abs(obj.V.current.value*2-200)-12+"px",
      obj.VS.current.value*3.6-12+"px"
  ])
}

///////////////////////////// text color /////////////////////////////
export const textColor=(obj,stts)=>{

  parseFloat(obj.L.current.value)<=50? stts.setTextColor1(true) : stts.setTextColor1(false)
}

///////////////////////////// BuiltIn color /////////////////////////////

export const builtInColor=(obj,stts)=>{
  stts.setBuiltInColorStt(["--",null])
  Object.keys(stts.builtInColors).forEach((val)=>{
      // console.log("val",builtInColors[val]["hexa"])
      
      if(obj.Hexa.current.value==stts.builtInColors[val]["hexa"]){
        stts.setBuiltInColorStt([val,stts.builtInColors[val]["hexa"]])
      }
      // console.log("BCStt",builtInColorStt)
  })
}
